...
import { forgotPassVerify } from "../../apis/apis";
  ...
  const [code, setCode] = useState("")
    ...
    const handleVerify = async() => {
      ...
      try {
          let _id=user?._id
          let payload={resetCode:code,_id}
        setVerifyLoader(true);
        let verified= await forgotPassVerify(payload)
        if(verified.status == 200){
          navigation.navigate("Forget1",{user:verified.data?.user});
