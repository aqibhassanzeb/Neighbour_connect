...
import { acceptRequest } from "../apis/apis";

const { width } = Dimensions.get("window");

const EditProfileScreen = (props) => {
  const [showFirstIcon, setShowFirstIcon] = useState(true);
  const sender = props.route.params?.user || {};
  ...
  const handleAccept = async () => {
    try {
      let accepted = await acceptRequest({ sender_id: sender.sender._id });
        ...
        <TouchableOpacity
          onPress={() =>
            props.navigation.navigate("ChattingScreen", {
              user: {
                recepientId: sender.sender._id,
                recepientName: sender.sender.name,
                recepientImage: sender.sender.image,
