...
function AddCat({ open, handleClose }) {
  const [addSell, addResp] = useAddSellCatMutation();
...
export default function SellCat() {
  const { data, isLoading } = useGetSellsCatsQuery();

  const [updateCat] = useUpdateSellCatMutation();
    ...
    <>
      ...
      {data && data.data.length > 0 && (
        <Box sx={{ paddingLeft: 3, paddingRight: 3 }}>
          <ImageList sx={{ width: 1030, height: 500 }} cols={6}>
            ...
            {data?.data.map((item) => (
              <ImageListItem key={item._id} sx={{ marginBottom: -1 }}>
                <div
                  style={{
                    padding: 10,
                    display: "flex",
